...
<head>
    ...
    <style type="text/css">
		...
		table.compare-list tr.price td .old-price .amount {
		...
		table.compare-list tr.price td .special-price .amount {
			font-size: 18px;
		}
...
<div id="yith-woocompare" class="woocommerce">
    ...
    <table id="datatable" class="compare-list" cellpadding="0"
        ...
        <?php else : ?>
			...
			if(sizeof($fields) == 0) {
				$fields = array(
					'image' => __( 'Image', 'cosmeli' ),
					'title' => __( 'Title', 'cosmeli' ),
					'price' => __( 'Price', 'cosmeli' ),
					'add-to-cart' => __( 'Add to cart', 'cosmeli' ),
					'description' => __( 'Description', 'cosmeli' ),
					'sku'           => __( 'Sku', 'cosmeli' ),
					'stock' => __( 'Availability', 'cosmeli' ),
					'weight'        => __( 'Weight', 'cosmeli' ),
					'dimensions'    => __( 'Dimensions', 'cosmeli' )
				);
			}?>
                    ...
                    foreach ($products as $product_id => $product) :
                        ...
                        <td class="<?php echo esc_attr( $product_class ); ?>"><?php
							if ($product->fields['title'] == '') {
								$product->fields['title'] = $product->get_title();
							}
							if ($product->fields['price'] == '') {
								$product->fields['price'] = $product->get_price_html();
							}
							if ($product->fields['description'] == '') {
								$description = apply_filters( 'woocommerce_short_description', yit_get_prop( $product, 'post_excerpt' ) );
								$product->fields['description'] = apply_filters( 'yith_woocompare_products_description', $description );
							}
							if ($product->fields['stock'] == '') {
								$availability = $product->get_availability();
								if ( empty( $availability['availability'] ) ) {
									$availability['availability'] = esc_html__( 'In stock', 'cosmeli' );
								}
								$product->fields['stock'] = sprintf( '<span>%s</span>', esc_html( $availability['availability'] ) );
							}
							if ($product->fields['sku'] == '') {
								$sku = $product->get_sku();
								! $sku && $sku = '-';
							...
							if ($product->fields['weight'] == '') {
								...
								$product->fields['weight'] = sprintf( '<span>%s</span>', esc_html( $weight ) );
							}
							if ($product->fields['dimensions'] == '') {
								...
								$product->fields['dimensions'] = sprintf( '<span>%s</span>', esc_html( $dimensions ) );
							}
